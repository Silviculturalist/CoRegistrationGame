...
def start_program(
    ...
    App(main_window, MyData, MyCHM, MyPlotCenters, startup_root=root, output_folder=output_folder)
...
def on_start():
    ...
    if not allow_overwrite_var.get() and (
        os.path.exists(trees_filepath) or os.path.exists(trans_filepath)
    ):
        files_at_risk = []
        ...
        warning_message = "\n".join(
            [
                ...
                *files_at_risk,
                "",
                (
                    "Please either check the 'Allow overwriting existing files' box or change "
                    "the Stand ID to a unique value."
                ),
            ]
        )
    ...
    for field in required_fields:
        if file_column_mapping[field] == "":
            messagebox.showerror(
                "Missing Column", f"Required column '{field}' is not mapped in the Tree Data File."
            )
            return
    if file_column_mapping["DBH"] == "" and file_column_mapping["H"] == "":
        messagebox.showerror(
            "Missing Column", "At least one of 'DBH' or 'H' must be mapped in the Tree Data File."
        )
        return
    ...
    for field in required_fields:
        if chm_column_mapping[field] == "":
            messagebox.showerror(
                "Missing Column", f"Required column '{field}' is not mapped in the CHM Data File."
            )
            return
    if chm_column_mapping["DBH"] == "" and chm_column_mapping["H"] == "":
        messagebox.showerror(
            "Missing Column", "At least one of 'DBH' or 'H' must be mapped in the CHM Data File."
        )
        return
    ...
    params = [float(param_entry_1.get()), float(param_entry_2.get()), float(param_entry_3.get())]
...
def on_file_sep_change(*args):
    update_column_options(
        file_path_entry.get(), file_sep_var.get(), file_comboboxes, file_mapping_vars
    )
...
ttk.Checkbutton(
    overwrite_frame, text="Allow overwriting existing files", variable=allow_overwrite_var
).pack(side=tk.LEFT)
...
ttk.Button(file_path_frame, text="Browse", command=select_file_path).grid(
...
file_sep_combobox = ttk.Combobox(
    file_path_frame, textvariable=file_sep_var, state="readonly", values=[",", ";", "\t", "|"]
)
...
for i, label in enumerate(["PlotID", "TreeID", "X", "Y", "DBH", "H"]):
    ...
    file_comboboxes[label] = ttk.Combobox(
        file_path_frame, textvariable=file_mapping_vars[label], state="readonly"
    )
    ...
    if label == "DBH":
        ...
        ttk.Checkbutton(
            file_path_frame,
            text="Impute DBH",
            variable=file_dbh_calc_var,
            command=toggle_file_impute_dbh,
        ).grid(column=2, row=i + 2, padx=5, pady=2)
    elif label == "H":
        file_h_calc_var = tk.BooleanVar()
        ttk.Checkbutton(
            file_path_frame,
            text="Impute Height",
            variable=file_h_calc_var,
            command=toggle_file_impute_h,
        ).grid(column=2, row=i + 2, padx=5, pady=2)
...
ttk.Button(chm_path_frame, text="Browse", command=select_chm_path).grid(
    column=2, row=0, padx=10, pady=5
)

ttk.Label(chm_path_frame, text="CSV Separator:").grid(column=0, row=1, padx=10, pady=5)
chm_sep_var = tk.StringVar(value=",")
chm_sep_combobox = ttk.Combobox(
    chm_path_frame, textvariable=chm_sep_var, state="readonly", values=[",", ";", "\t", "|"]
)
...
for i, label in enumerate(["PlotID", "TreeID", "X", "Y", "DBH", "H"]):
    ...
    chm_comboboxes[label] = ttk.Combobox(
        chm_path_frame, textvariable=chm_mapping_vars[label], state="readonly"
    )
    ...
    if label == "DBH":
        ...
        ttk.Checkbutton(
            chm_path_frame,
            text="Impute DBH",
            variable=chm_dbh_calc_var,
            command=toggle_chm_impute_dbh,
        ).grid(column=2, row=i + 2, padx=5, pady=2)
    elif label == "H":
        ...
        ttk.Checkbutton(
            chm_path_frame,
            text="Impute Height",
            variable=chm_h_calc_var,
            command=toggle_chm_impute_h,
        ).grid(column=2, row=i + 2, padx=5, pady=2)
...
ttk.Button(output_folder_frame, text="Browse", command=select_output_folder).grid(
    column=2, row=0, padx=10, pady=5
)
...
ttk.Label(
    params_frame, text="Units: supply DBH in centimeters; model returns height in meters."
).grid(column=0, row=3, columnspan=2, padx=5, pady=(6, 2), sticky="w")
...
def update_params_section():
    ...
    if (
        file_dbh_calc_var.get()
        or file_h_calc_var.get()
        or chm_dbh_calc_var.get()
        or chm_h_calc_var.get()
    ):
        params_frame.grid()
        graph_frame.grid()
        plot_params_graph()
